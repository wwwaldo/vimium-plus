...
class SearchController {
  ...
  createResultItem(result, isSelected) {
    ...
    const { title, href, ariaLabel, text, innerText, displayText } = element.metadata;
    
    // Choose best text to display
    const displayContent = displayText || innerText || text || title || href || ariaLabel || 'Unnamed Element';
  ...
  search(query) {
    ...
    const results = this.navigator.clickableElements.map((element, index) => {
      const { title, href, ariaLabel, text, innerText, displayText } = element.metadata;
      const searchText = [title, href, ariaLabel, text, innerText, displayText]
