...
class SearchController {
  ...
  getTypeIndicator(element) {
    switch (element.metadata.tagName) {
  ...
  createResultItem(result, isSelected) {
    ...
    const { tagName, title, href, ariaLabel, text, innerText, displayText } = element.metadata;
  ...
  search(query) {
    ...
    console.log('Available elements:', this.navigator.clickableElements);

    // Special case: * shows all elements
    if (query === '*') {
      this.currentResults = this.navigator.clickableElements.map((element, index) => ({
        element,
        index,
    ...
    const results = this.navigator.clickableElements.map((element, index) => {
      const { title, href, ariaLabel, text, innerText, displayText, tagName } = element.metadata;
