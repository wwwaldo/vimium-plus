...
class ElementNavigator {
  constructor() {
    // Singleton pattern
    if (window.vimiumNavigator) {
      return window.vimiumNavigator;
    }
  ...
  async click() {
    ...
    if (element.tagName === 'INPUT' || element.tagName === 'TEXTAREA' || element.tagName === 'SELECT' || element.contentEditable === 'true') {
      element.focus();
    }
...
window.vimiumNavigator = new ElementNavigator();
