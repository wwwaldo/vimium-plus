...
class KeyboardController {
  constructor(deps = {}) {
    ...
    this.navigator = deps.navigator || window.vimiumNavigator;
    ...
    document.addEventListener('keydown', (e) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'l':  // Forward in history
          ...
          break;
        case 'y':  // Yank visible text
          if (!e.shiftKey && !e.altKey && !e.ctrlKey && !e.metaKey) {
            e.preventDefault();
            if (window.viewportUtils.copyVisibleText()) {
              this.showStatusMessage('Copied visible text to clipboard');
            } else {
              this.showStatusMessage('Failed to copy text');
            }
          }
          break;
        case 'y':  // Yank visible text
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  if (!window.navigator || !window.searchController || !window.recorder || !window.highlighter) {
    throw new Error('Required dependencies not found! Make sure all scripts are loaded.');
  }
