...
class KeyboardController {
  constructor(deps = {}) {
    ...
    this.highlighter = deps.highlighter || new ElementHighlighter();
    this.highlightsVisible = false;  // Start with highlights off
    this.lastKeyPressTime = null;
    this.initializeKeyHandlers();

    // Listen for element updates from highlighter
    this.highlighter.onReadyCallbacks.push(() => {
      if (window.clickableElements) {
        this.navigator.setElements(window.clickableElements);
      }
    });
  }
  ...
  initializeKeyHandlers() {
    document.addEventListener('keydown', (e) => {
      ...
      // In search mode, only handle Escape key
      if (this.searchController.searchMode) {
        if (e.key === 'Escape') {
          this.searchController.exitSearch();
      ...
      switch(e.key) {
        ...
        case 'Escape':
          ...
          e.preventDefault();
          break;
        case 'f':  // Toggle highlights
          if (!e.shiftKey && !e.altKey && !e.ctrlKey && !e.metaKey) {
            this.toggleHighlights();
            this.showStatusMessage(this.highlightsVisible ? 'Highlights On' : 'Highlights Off');
            e.preventDefault();
          }
          break;
