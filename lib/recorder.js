...
class InteractionRecorder {
  constructor(navigator) {
    ...
    this.recording = this.loadRecordingState();
  ...
  toggleRecording() {
    ...
    this.saveRecordingState();
    this.updateStatus(`Recording ${this.recording ? 'started' : 'stopped'}`);
  }

  loadSequence() {
    const saved = localStorage.getItem('vimium_recording');
    return saved ? JSON.parse(saved) : [];
  }

  saveSequence() {
    localStorage.setItem('vimium_recording', JSON.stringify(this.sequence));
  }

  loadRecordingState() {
    return localStorage.getItem('vimium_is_recording') === 'true';
  }

  saveRecordingState() {
    localStorage.setItem('vimium_is_recording', this.recording);
  }

  initialize() {
    // Keyboard events now handled by KeyboardController
  }
}
