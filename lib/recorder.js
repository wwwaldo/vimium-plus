...
class InteractionRecorder {
  ...
  startRecording() {
    this.recording = true;
    this.updateStatus('⏺️ Recording...');
    console.log('⏺️ Started recording interaction sequence');
  }
  ...
  recordInteraction(element) {
    ...
    this.updateStatus(`⏺️ Recorded ${this.sequence.length} ${this.sequence.length === 1 ? 'action' : 'actions'}`);
  }
  ...
  async startPlayback(delay = 1000) {
    if (this.sequence.length === 0) {
      console.log('⚠️ No recorded sequence to play');
      return;
    }
    
    this.playbackIndex = 0;
    this.updateStatus('▶️ Playing sequence...');
    
    while (this.playbackIndex < this.sequence.length) {
      const fingerprint = this.sequence[this.playbackIndex];
      ...
      if (element) {
        ...
        this.updateStatus(`▶️ Action ${this.playbackIndex + 1}/${this.sequence.length}`);
      } else {
        console.log('⚠️ Could not find matching element for:', fingerprint);
      }
...
window.recorder = new InteractionRecorder(window.navigator);
